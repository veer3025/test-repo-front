...
export async function POST(req: any) {
  ...
  try {
    ...
    if(form_data) {
      ...
      var template_id  = form_data.get('template_id')??0; 
      ...
      if(is_err === 0) {
        ...
        qry = `UPDATE yearbooks SET steps = ? , template_id = ?, modified = ? WHERE id = ?`;
        
        val = [steps, template_id, str_dt, year_book_id];
